...
"""
    ...
    for noteObj in SQLconn.execute(
            f"SELECT NoteID from notes where UserID=\"{liveCacheManager.getUserID(liveCacheManager.ByViewerID, viewerObj.viewerID)}\""):
        noteObj = SQLconn.execute(
            f"SELECT SubjectID, Header, Description from note_relevance where NoteID=\"{noteObj['NoteID']}\" limit 1")[
            0]
...
class Player:
    def __init__(self, viewer: BaseViewer | None = None):
        ...
        self.userName = liveCacheManager.getUserName(liveCacheManager.ByViewerID,
                                                     viewer.viewerID) if viewer else self.userName
        self.userID = liveCacheManager.getUserID(liveCacheManager.ByViewerID,
                                                 viewer.viewerID) if viewer else self.userID
...
class Party:
    def __init__(self, teamCount: int, playerCount: int):
        ...
        while not self.partyID or self.partyID in liveCacheManager.activeParties: self.partyID = StringGenerator().AlphaNumeric(
            5, 30)
        ...
        if not self.timerInitialised:
            ...
            while time() - partyTimerStartedAt < 4 and sum([len(team.players) for team in self.teams]) > 1:
                for player in self.userIDToPlayer.values():
                    player.viewerObj.queueTurboAction(str(4 - int(time() - partyTimerStartedAt)), "queueTimer",
                                                      player.viewerObj.turboApp.methods.update)
                sleep(1)
    ...
    def joinTeam(self, viewer: BaseViewer | None = None):
        ...
        if self.gameObj is None and sum([len(team.players) for team in self.teams]) < 6:
            ...
            if player.isHuman:
                if sum([len(team.players) for team in self.teams]) == 6:
                    self.partyComplete()
                else:
                    Thread(target=self.startTimer).start()
            return True
        return False

    def leaveTeam(self, viewer: BaseViewer, team: Team | None = None):
        player = self.userIDToPlayer.get(liveCacheManager.getUserID(liveCacheManager.ByViewerID, viewer.viewerID),
                                         Player(viewer))
...
class Quiz:
            ...
            if playerToRenderFor.isHuman:
                for playerToRender in self.party.userIDToPlayer.values():
                    ...
                    if playerToRenderFor.team == playerToRender.team:
                        playerToRenderFor.viewerObj.queueTurboAction(playerDiv, "selfTeam",
                                                                     playerToRenderFor.viewerObj.turboApp.methods.newDiv)
                    else:
                        playerToRenderFor.viewerObj.queueTurboAction(playerDiv, "otherTeam",
                                                                     playerToRenderFor.viewerObj.turboApp.methods.newDiv)
    ...
    def initQuestions(self):
        for questionData in SQLconn.execute("SELECT * from questions where QuizEligible=1 ORDER BY RAND() LIMIT 30"):
            question = Question()
            question.setValues(self.party, questionData["QuestionID"], questionData["Text"],
                               loads(questionData["Options"]))
    ...
    def nextQuestion(self):
        ...
        for player in self.party.userIDToPlayer.values():
            ...
            player.viewerObj.queueTurboAction(currentQuestion.questionStatement, "questionText",
                                              player.viewerObj.turboApp.methods.update.value)
            options = ""
    ...
    def receiveUserInput(self, isHuman: bool, viewer: BaseViewer | Player, optionIndex):
        ...
        currentQuestion = self.questions[self.questionIndex]
        if len(currentQuestion.teamOptions[
                   player.team.teamID]) > optionIndex >= 0 and player not in currentQuestion.optionsPressed:
            currentQuestion.optionsPressed[player] = optionIndex
            if currentQuestion.teamOptions[player.team.teamID][optionIndex] in currentQuestion.correctAnswers:
                player.score += 10
            else:
                player.score -= 5
            if isHuman:
    ...
    def endCurrentQuestion(self):
        ...
        for player in currentQuestion.optionsPressed:
            ...
            if currentQuestion.teamOptions[player.team.teamID][
                currentQuestion.optionsPressed[player]] in currentQuestion.correctAnswers:
                points[player.team.teamID] += 1
            else:
                points[player.team.teamID] -= 1
        ...
        if points[teamA.teamID] < points[teamB.teamID]: self.updateTeamHealth(teamA, 10 * (1 + self.questionIndex) * (
                    points[teamA.teamID] - points[teamB.teamID]))
        self.nextQuestion()
    ...
    def updateTeamHealth(self, teamChanged: Team, offset):
        ...
        for player in self.party.userIDToPlayer.values():
            ...
            if player.team == teamChanged:
                player.viewerObj.queueTurboAction(bar, f"selfTeamHealthBar",
                                                  player.viewerObj.turboApp.methods.update.value)
                player.viewerObj.queueTurboAction(str(teamChanged.health), f"selfTeamHealthText",
                                                  player.viewerObj.turboApp.methods.update.value)
            else:
                player.viewerObj.queueTurboAction(bar, f"otherTeamHealthBar",
                                                  player.viewerObj.turboApp.methods.update.value)
                player.viewerObj.queueTurboAction(str(teamChanged.health), f"otherTeamHealthText",
                                                  player.viewerObj.turboApp.methods.update.value)
        ...
        if teamChanged.health == 0:
            ...
            for player in self.party.userIDToPlayer.values():
                if player.isHuman:
                    ...
                    if player.team == teamChanged:
                        player.viewerObj.queueTurboAction("DEFEAT", "resultTextDiv",
                                                          player.viewerObj.turboApp.methods.update)
                    else:
                        player.viewerObj.queueTurboAction("VICTORY", "resultTextDiv",
                                                          player.viewerObj.turboApp.methods.update)
                    player.viewerObj.queueTurboAction(leaderboardDiv, "quizLeaderboard",
                                                      player.viewerObj.turboApp.methods.update)
...
class UserCache:
    ...
    def loginCall(self, viewer: BaseViewer, userID):
        ...
        received = SQLconn.execute(
            f"SELECT UserID, RemoteAddr, UserAgent, HostURL from user_devices where ViewerID=\"{viewer.viewerID}\"")
        addEntry = False
        if received:
            received = received[0]
            if received["UserID"].decode() != userID or received["RemoteAddr"] != viewer.cookie.remoteAddress or \
                    received["UserAgent"] != viewer.cookie.UA or received["HostURL"] != viewer.cookie.hostURL:
        ...
        if addEntry:
            SQLconn.execute(
                f"INSERT INTO user_devices values (\"{viewer.viewerID}\", \"{userID}\", \"{viewer.cookie.remoteAddress}\", \"{viewer.cookie.UA}\", \"{viewer.cookie.hostURL}\")")
    ...
    def getKnownLoggedInUserID(viewer: BaseViewer):
        ...
        if received:
            received = received[0]
            if remoteAddr == received["RemoteAddr"] and userAgent == received["UserAgent"] and hostURL == received[
                "HostURL"]:
...
def registerUser(viewerObj: BaseViewer, form: dict):
    ...
    if not username:
        ...
        viewerObj.queueTurboAction("Passwords Not Valid", "registrationWarning",
...
try:
    ...
    WSGIServer(('0.0.0.0', ServerSecrets.webPort.value,), baseApp, log=None, keyfile=r'C:\cert\privkey.pem',
               certfile=r'C:\cert\cert.pem').serve_forever()
except:
